...
class Macro:
    ...
    def __repr__(self) -> str:
        used = "*" if self.used else ""
        options = self.options or ""
        return f"Macro({used}[{self.level}] {self.name}{options} {self.body})"
...
class RPM:
    @staticmethod
    def parse(
        s: str, sourcedir: Path, macros: Optional[List[Tuple[str, str]]] = None
    ) -> rpm.spec:
        """
        Parses the content of a spec file.

        Args:
            s: String representing the content of a spec file.
